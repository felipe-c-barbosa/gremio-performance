...
import type { ComparisonYear, PositionRow } from "@/lib/data";
        ...
        {payload
          ...
          .map((p) => (
            <li key={p.name} className="flex justify-between gap-4 tabular-nums">
              ...
              <span className="text-zinc-200">{p.value}º</span>
            </li>
          ))}
      </ul>
    </div>
...
export function PositionChart({ data, hiddenYears, onLegendClick }: Props) {
  return (
    <div className="space-y-3">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            ...
            <YAxis
              ...
              tick={{ fill: "#a1a1aa", fontSize: 11 }}
              label={{
                value: "Posição na tabela (1º = topo)",
                angle: -90,
                position: "insideLeft",
                fill: "#71717a",
                fontSize: 11,
      ...
      <p className="text-xs text-zinc-500">
        Eixo Y invertido: mais alto na tela = melhor posição (1º). A posição reflete a
        tabela após cada rodada, recalculada com todos os jogos do campeonato.
      </p>
    </div>
  );
}
